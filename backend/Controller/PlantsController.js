...
exports.showPlants = async (req, res) => {
  ...
  res.status(201).json({ success: true, message: "All plants", payload });
};
...
exports.onePlant = async (req, res) => {
  ...
  res.status(201).json({ success: true, message: "one plant", payload });
};

exports.deletePlant = async (req, res) => {
  let payload = await PlantsSchema.deleteOne({ _id: req.params.id });
  res.status(201).json({ success: true, message: "Deleted", payload });
};
